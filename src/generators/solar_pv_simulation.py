...
def recv_climate_data(sockobj):
    ...
    while True:
        ...
        tmp_modeled_power = obj_sim.generated_power_point(real_data['temp'], real_data['ws'], real_data['radiation'])
...
def simulation(ax, fig, line, line2, canvas):
    ...
    ax.axis([sim_timestamp[m_left_index], sim_timestamp[m_right_index], 0, 2000])
    ...
    print("sim_timestamp, modeled_power", list(sim_timestamp), list(modeled_power))
    canvas.show()
    # print(sim_timestamp[-10:])
    root.after(1000, simulation,ax, fig, line, line2, canvas)
...
class SolarPVSim(object):
    ...
    def generated_power_point(self, temp, wind, power):
        VA_Outs = []
        Tambient = temp
...
if __name__ == "__main__":
    ...
    recv_cli_data = Process(name='start connection to receive data from server', target=recv_climate_data, args=(sockobj,))
    recv_cli_data.start()
